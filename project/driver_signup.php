...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $query    = "INSERT into `drivers` (name, surname, drivingLicenseNum, carModel, licensePlate, email, phone, password)
                     VALUES (?,?,?,?,?,?,?,?)";
         $stmt = mysqli_stmt_init($conn);
         $prepareStmt = mysqli_stmt_prepare($stmt,$query);
         if ($prepareStmt) {
             mysqli_stmt_bind_param($stmt,"ssssssss",$name, $surname, $drivingLicenseNum, $carModel, $licensePlate, $email, $phone, $password);
             mysqli_stmt_execute($stmt);
             header("Location: driver_login.php");
             exit; 
         }else{
             die("Click here to <a href='driver_signup.php'>sign up</a> again.");
         }
    } 
    else {
?>
...
<div class="registration">
    ...
    <input type="submit" class="btn mt-3" name="submit" value="Sign up">
            ...
            <div class="text-center fs-6">
                <p>Already have an account? <a href="driver_login.php"><u>Log in</u></a></p>
            </div>
    </form>
