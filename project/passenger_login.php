<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $query = "SELECT * FROM `passengers` WHERE phone='$phone' AND password='$password'";
        $result = mysqli_query($conn, $query) or die(mysql_error());
        $rows = mysqli_num_rows($result);
        if ($rows == 1) {
            // Generate a custom session ID
            $session_id = generateCustomSessionId(); // You need to define this function
            ...
            $user_id = $user_data['passenger_id'];
    ...
    <div class="loginform">
        <form class="p-3 mt-3" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="login">
            ...
            <div class="text-center fs-6">
                <p>Don't have an account? <a href="passenger_signup.php"><u>Sign up</u></a></p>
            </div>
        </form>
    </div>
</div>
